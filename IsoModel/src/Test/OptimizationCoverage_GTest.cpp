...
// Tests the change from std::string to const std::string& and pointer arithmetic
TEST(OptimizationCoverage, EpwData_Parsing) {
    ...
    // Test Data Parsing
    // We need to construct a CSV line where specific indices map to specific columns
    // Indices: DBT=6(0), DPT=7(1), RH=8(2), EGH=13(3), EB=14(4), ED=15(5), WSPD=21(6)
    
    // Constructing a line with enough commas to reach index 21
    // 0,1,2,3,4,5, 6(DBT), 7(DPT), 8(RH), 9,10,11,12, 13(EGH), 14(EB), 15(ED), 16,17,18,19,20, 21(WSPD)
    // Note: Added trailing comma/dummy field because parseData loop requires a trailing comma to process the field
    std::string dataLine = "1999,1,1,1,0,?, -12.2, -16.1, 72, 0,0,0,0, 500, 800, 100, 0,0,0,0,0, 2.6, 0";
...
TEST(OptimizationCoverage, UserModel_ModernFeatures) {
    ...
    // Test [[nodiscard]] getters (compile-time check mostly, but runtime check here)
