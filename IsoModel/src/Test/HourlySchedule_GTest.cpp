...
// hourly results against a set of known, expected values.
...
TEST_F(ISOModelFixture, HourlyScheduleTests) {
  ...
  for (size_t i = 0; i < results.size() && std::getline(expectedFile, line); ++i) {
    ...
    if (iss >> hour >> expectedHeating >> expectedCooling >> expectedLighting >> expectedEquipment) {
...
#else
      double actualHeating = results[i].getEndUse(EndUseFuelType::Electricity, EndUseCategoryType::Heating);
      double actualCooling = results[i].getEndUse(EndUseFuelType::Electricity, EndUseCategoryType::Cooling);
      double actualLighting = results[i].getEndUse(EndUseFuelType::Electricity, EndUseCategoryType::InteriorLights);
      double actualEquipment = results[i].getEndUse(EndUseFuelType::Electricity, EndUseCategoryType::InteriorEquipment);
#endif
