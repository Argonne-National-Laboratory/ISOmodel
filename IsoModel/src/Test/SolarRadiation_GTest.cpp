...
// ASHRAE Fundamentals, Ch. 14 Climatic Design Information (with the exception
// of using Duffie and Beckman p.14, eq. 1.6.1b for declination). An ipython
// notebook of the hand calculations is saved in the test_data directory.
...
TEST_F(ISOModelFixture, SunPositionAndRadiationTests) {
  ...
  // The lat, lon and meridian are based on the weather file. The tilt is
  // currently defaulted to pi/2.
  ...
  // Confirm the radiation values for 12noon, Jan 21 are what we expect them to
  // be:
  ...
  auto diffuseAngleOfIncidenceFactor =
      solarRadiation.calculateDiffuseAngleOfIncidenceFactor(angleOfIncidence);
