    ...
    namespace isomodel {
            ...
            Vector& v_Tdbt_nt, Vector& v_Tdbt_Day) const
        {
            ...
            for (int i = 0; i < monthsInYear; i++) {
                ...
                v_frac_hrs_sun_up[i] = (v_sun_down_time[i] - v_sun_up_time[i] + 1) / hoursInDay;
            ...
            Vector& v_Q_illum_tot, Vector& v_Q_illum_ext_tot) const
        {
            ...
            Q_illum_occ = structure.floorArea() * lpd_occ * F_C * F_O * (t_lt_D * F_D + t_lt_N) * W2kW;
            // Total annual lighting energy for unnocupied times (kWh).
            Q_illum_unocc = structure.floorArea() * lpd_unocc * t_unocc * W2kW;
            ...
            v_Q_illum_ext_tot = mult(v_hrs_sun_down_mo, lights.exteriorEnergy() * W2kW);
        }
            ...
            double& phi_plug_avg, double& phi_illum_avg, double& phi_int_wke_nt, double& phi_int_wke_day, double& phi_int_wk_nt) const
        {
            ...
            double phi_illum_occ = Q_illum_occ / structure.floorArea() / hoursInYear / frac_hrs_wk_day * kW2W; //
            double phi_illum_unocc = Q_illum_unocc / structure.floorArea() / hoursInYear / (1 - frac_hrs_wk_day) * kW2W;
            phi_illum_avg = Q_illum_tot_yr / structure.floorArea() / hoursInYear * kW2W;
            ...
            double frac_hrs_wke_tot, Vector& v_Th_avg, Vector& v_Tc_avg, double& tau) const
        {
            ...
            // the weekend is two days (we'll call them sat and sun)u
            //
