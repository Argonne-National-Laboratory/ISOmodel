...
public:
  ...
  double calculateRevolutionAngle(int dayOfYear) const { return 2.0 * PI * dayOfYear / DAYS_IN_YEAR; }
  ...
  double calculateEquationOfTime(double B) const {
  ...
  double calculateApparentSolarTime(int localStandardTime, double equationOfTime) const {
  ...
  double calculateSolarDeclination(double B) const {
  ...
  double calculateSolarHourAngle(double ast) const { return (ast - 12) * (PI / 12.0); }

  // Calculates the solar altitude angle (beta) in radians.
  // Ref: ASHRAE Fundamentals 2013, Ch 14, Eq 6
  // Ref: Duffie & Beckman Eq 1.6.5
  double calculateSolarAltitude(double dec, double sha) const {
  ...
  double calculateSolarAzimuthSin(double dec, double H, double beta) const {
    return std::sin(H) * std::cos(dec) / std::cos(beta);
  }
  double calculateSolarAzimuthCos(double dec, double H, double beta) const {
    return (std::cos(H) * std::cos(dec) * std::sin(m_latitude) -
            std::sin(dec) * std::cos(m_latitude)) /
           std::cos(beta);
  }
  double calculateSolarAzimuth(double sina, double cosa) const { return std::atan2(sina, cosa); }
  ...
  double calculateSurfaceSolarAzimuth(double solAz, double surfAz) const {
  ...
  double calculateAngleOfIncidence(double beta, double gamma, double tilt) const {
  ...
  double calculateTotalDirectBeamIrradiance(double eb, double theta) const {
    return eb * std::max(std::cos(theta), 0.0);
  }

  // Diffuse angle of incidence factor (Y)
  // Ref: ASHRAE Fundamentals 2013, Ch 14, Eq 22
  double calculateDiffuseAngleOfIncidenceFactor(double theta) const {
  ...
  double calculateTotalDiffuseIrradiance(double ed, double Y, double tilt) const {
    return (tilt > PI / 2) ? ed * Y * std::sin(tilt) : ed * (Y * std::sin(tilt) + std::cos(tilt));
  }

  // Total irradiance (Global Surface)
  double calculateTotalIrradiance(double dir, double diff, double ground) const {
    return dir + diff + ground;
  }

  // --- Getters with Legacy Interface Support ---

  double surfaceTilt() const { return m_surfaceTilt; }
  double localMeridian() const { return m_localMeridian; }
  double lon() const { return m_longitude; }
  double lat() const { return m_latitude; }
  double groundReflectance() const { return m_groundReflectance; }

  // Reconstructs 2D vector on demand for legacy support
  std::vector<std::vector<double>> eglobe() const;
  ...
  std::vector<double> monthlyGlobalHorizontalRadiation() const {
    return m_monthlyGlobalHorizontalRadiation;
  }

  // Legacy getters that reconstruct 2D vectors from flat storage
  std::vector<std::vector<double>> monthlySolarRadiation() const;
  std::vector<std::vector<double>> hourlyDryBulbTemp() const;
  std::vector<std::vector<double>> hourlyDewPointTemp() const;
  std::vector<std::vector<double>> hourlyGlobalHorizontalRadiation() const;
};
