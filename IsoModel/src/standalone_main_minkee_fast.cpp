...
std::vector <std::string> ismvector;
//end added headers
...
{
    //minkee's use of boost libraries to search folder for files

    path p("C:/ISOmodel/onlyISMS");
    for (auto i = directory_iterator(p); i != directory_iterator(); i++)
    {
        if (!is_directory(i->path())) //ignore directories
        {
            //cout << i->path().filename().string() << endl;
            if(i->path().filename().string() != "ORD.epw") //any file undesired
                  ismvector.push_back(i->path().filename().string());
        }
        else
            continue;
    }
/*
    for(int i = 0; i < ismvector.size(); i++){
          cout << ismvector[i] << endl;
   }*/

    //end minkee's boost call
  ...
  if (vm.count("defaultsfilepath")) {
    ...
    //umodel.loadJustBuilding(vm["ismfilepath"].as<std::string>());
    //umodel.loadJustWeather(vm["ismfilepath"].as<std::string>());
    bool loop = true;
    umodel.loadJustBuilding(ismvector[arrayindex]);
    umodel.loadJustWeather(ismvector[arrayindex]);
    //umodel.load(vm["ismfilepath"].as<std::string>());
  }
  while(loop == true && arrayindex < ismvector.size()){
        umodel.loadJustBuilding(ismvector[arrayindex]);
