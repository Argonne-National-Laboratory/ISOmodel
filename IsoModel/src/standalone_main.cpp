...
void runMonthlySimulation(const UserModel &umodel) {
  ...
  std::cout << "Monthly "
               "Results:\nMonth,ElecHeat,ElecCool,ElecIntLights,ElecExtLights,"
               "ElecFans,ElecPump,ElecEquipInt,ElecEquipExt,ElectDHW,GasHeat,"
...
void runHourlySimulation(const UserModel &umodel, bool aggregateByMonth) {
  ...
  for (size_t i = 0; i < hourlyResults.size(); i++) {
    ...
    // Fill remaining columns (GasCool, GasEquip, GasDHW) with 0 if not present
    // in getEndUse
...
void compare(const UserModel &umodel, bool markdown = false) {
  ...
  auto endUseNames = std::vector<std::string>{
      "ElecHeat", "ElecCool",     "ElecIntLights", "ElecExtLights", "ElecFans",
